...
public class XanxusPlayer extends Player {
     ...
     */
    public XanxusPlayer(long seed, int id) {
        super(seed, id);
        reset(seed, id);
    }
    ...
    @Override
    public Player copy() {
        XanxusPlayer player = new XanxusPlayer(seed,playerID);
    ...
    public Types.ACTIONS act(GameState gs) {
        ...
        // I think this method is not suitable. sometime we need back and forth.
    ...
    private void rollRnd(GameState gs, Types.ACTIONS act)
    {
        ...
        //Array of actions. One per player (size: 4)
        int nPlayers = 4;
        Types.ACTIONS[] actionsAll = new Types.ACTIONS[4];

        for(int i = 0; i < nPlayers; ++i)
        {
            //This line is identifying if the ID of the player corresponds to the position or index 'i' in the array.
            // Note that internally the IDs of the players are 10, 11, 12 and 13 (for each of the 4 players).
            // It's essentially doing the mapping "a -> b" ,
            // where "a" is the player ID (10, 11, 12, 13) and "b" is the position in the actionsAll array (0, 1, 2, 3).
            //Calculate the index where my action goes.
            if(i == getPlayerID() - Types.TILETYPE.AGENT0.getKey())
            {
                ...
                if(rndOpponentModel){
                    ...
                    actionsAll[i] = Types.ACTIONS.all().get(actionIdx);//Here I put the action I want to simulate.
                }else
                {
                    actionsAll[i] = Types.ACTIONS.ACTION_STOP;//Here I put the action I want to simulate.
                }
            }
        }
